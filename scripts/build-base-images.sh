...
podman build -f "$BASE_DIR/rust-builder.Dockerfile" --build-arg RUST_VERSION="$RUST_VERSION" -t "$RUST_BUILDER_IMAGE" "$ROOT_DIR"
podman build -f "$BASE_DIR/rust-runtime.Dockerfile" -t "$RUST_RUNTIME_IMAGE" "$ROOT_DIR"
podman build -f "$BASE_DIR/strategy-builder.Dockerfile" --build-arg RUST_VERSION="$RUST_VERSION" -t "$STRATEGY_BUILDER_IMAGE" "$ROOT_DIR"
podman build -f "$BASE_DIR/strategy-runtime.Dockerfile" -t "$STRATEGY_RUNTIME_IMAGE" "$ROOT_DIR"
podman build -f "$BASE_DIR/web-node.Dockerfile" --build-arg NODE_VERSION="$NODE_VERSION" -t "$WEB_BASE_IMAGE" "$ROOT_DIR"
