  ...
  return (
    <div className="relative mt-3 rounded-lg border border-border bg-background/60 p-3">
      <button
        ...
        className="absolute right-2 top-2 rounded-md border border-border bg-secondary/40 px-2 py-0.5 text-3xs uppercase tracking-wide text-muted-foreground hover:text-foreground"
      >
      ...
      {data && !loading && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Kpi
              label={t('kpiCorpus')}
              value={formatNumber(locale, data.corpus_ticks, 0)}
            />
            <Kpi
              label={t('kpiBaseline')}
            ...
            <Kpi
              label={t('kpiWin')}
              value={`${formatNumber(locale, data.baseline.win_rate_pct, 1)}%`}
            />
