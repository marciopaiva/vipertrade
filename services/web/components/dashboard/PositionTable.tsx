...
export function PositionTable({
  ...
  const formatRelativeOpenTime = (openedAt?: string) => {
    ...
    const diffSeconds = Math.max(0, Math.floor((nowMs - openedMs) / 1000));
  ...
  const enrichedPositions = positions.map(p => {
    ...
    const unrealizedPnl =
      typeof markPrice === 'number'
        ? (isLong ? markPrice - entryPrice : entryPrice - markPrice) *
          p.quantity
        : null;
    const unrealizedPnlPct =
      p.notional_usdt > 0 && typeof unrealizedPnl === 'number'
      ...
      typeof p.trailing_stop_final_distance_pct === 'number'
        ? isLong
          ? p.trailing_stop_peak_price *
            (1 - p.trailing_stop_final_distance_pct)
          : p.trailing_stop_peak_price *
            (1 + p.trailing_stop_final_distance_pct)
        : null;
    ...
    const trailingProgressPct =
      typeof markDeltaPct === 'number' &&
      typeof activationMovePct === 'number' &&
      activationMovePct > 0
    ...
    const trailingDisplayPrice =
      typeof trailingLivePrice === 'number'
        ? trailingLivePrice
        : typeof p.trailing_activation_price === 'number'
      ...
      <CardContent className="pt-0">
        ...
        <div className="space-y-2">
          {enrichedPositions.map(position => {
            const pnlColor =
              (position.unrealizedPnl ?? 0) >= 0 ? '#10b981' : '#ef4444';
            const sideColor =
              position.side.toLowerCase() === 'long' ? '#10b981' : '#ef4444';
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs md:grid-cols-4 xl:contents">
                    ...
                    {typeof position.trailingDisplayPrice === 'number' ? (
                      <Badge
                        className="h-6 w-full justify-center px-2 text-[11px] font-medium"
                        style={{
                          backgroundColor: position.trailing_stop_activated
                            ? '#10b98118'
                            : position.trailingArmedByPrice
                              ? '#f59e0b14'
                              : '#0f172acc',
                          color: position.trailing_stop_activated
                            ? '#86efac'
                            : position.trailingArmedByPrice
                              ? '#fcd34d'
                              : '#e2e8f0',
                          borderColor: position.trailing_stop_activated
                            ? '#10b98155'
                            : position.trailingArmedByPrice
                              ? '#f59e0b55'
                              : '#334155',
                          boxShadow: position.trailing_stop_activated
                            ...
                            width: `${
                              position.trailing_stop_activated
                                ...
                                : Math.max(
                                    6,
                                    Math.round(
                                      (position.trailingProgressPct ?? 0) * 100
                                    )
                                  )
                            }%`,
                          }}
                  ...
                  <div className="flex flex-wrap items-center gap-2 xl:justify-start">
                    ...
                    >
                      <span className="mr-1.5 text-[11px]">
                        {regimeIcon(position.consensusSide)}
                      </span>
                      Consensus
                    </Badge>
                    <Badge
                    ...
                    >
                      <span className="mr-1.5 text-[11px]">
                        {regimeIcon(position.bybitRegime)}
                      </span>
                      Bybit
                    </Badge>
                  </div>
