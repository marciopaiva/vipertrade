// English catalog. This file is the TYPE SOURCE of truth for messages — pt-BR.ts
// must satisfy the same shape. Flat namespaces; values may contain {var} placeholders
// interpolated by useT(). Keep keys stable; add a namespace as a screen is migrated.
const en = {
  nav: {
    primary: 'Primary',
    console: 'Command Deck',
    strategy: 'Strategy',
    trades: 'Trades',
    analysis: 'Analysis',
    system: 'System',
  },
  deck: {
    title: 'Command Deck',
    subtitle:
      'Live decisions, open risk and realized PnL — the screen to keep open while the strategy runs.',
    statusLive: 'LIVE · {n} assets',
    equity: 'Equity',
    sentiment: 'Sentiment',
    winRate: 'Win rate · 24h',
    net24h: 'net 24h',
    decisionMatrix: 'Decision matrix',
    liveFeed: 'Live feed',
    feedEmpty: 'No fills yet — the feed updates as trades close.',
  },
  common: {
    language: 'Language',
    density: 'Density',
    accountMenu: 'Account and preferences',
    logout: 'Log out',
    loading: 'Loading…',
    error: 'Error',
    generate: 'Generate',
    regenerate: 'Regenerate',
    running: 'Running…',
    copy: 'copy',
    copied: 'copied ✓',
    confirm: 'Confirm',
    cancel: 'Cancel',
    sure: 'Sure?',
  },
  analysis: {
    title: 'Analysis',
    subtitle:
      'Live operation quality (realized trades) and a deterministic tuning simulation (what-if). Config changes are applied manually.',
    tabLive: 'Live',
    tabWhatif: 'What-if (grid)',
  },
  live: {
    blurb:
      'Operation quality over realized (closed, paper) trades — real data, not a backtest. Refreshes every 30s.',
    kpiNet: 'Realized net PnL',
    kpiWin: 'Win rate',
    kpiClosed: 'Closed trades',
    kpiCapture: 'Peak capture',
    empty: 'No closed trades in the last {days}d.',
    followTitle: 'Entry follow-through · advanced vs stillborn',
    followArmed: 'Advanced (covered its cost)',
    followNotArmed: 'Stillborn (never in net profit)',
    peakTitle: 'Peak capture · trailing exits ({n})',
    peakAvg: 'avg peak',
    peakRealized: 'realized',
    peakLocked: 'locked',
    reasonsTitle: 'Attribution by exit reason',
    bySymbolTitle: 'By token · worst first',
    colSymbol: 'Symbol',
    colNet: 'net PnL',
    colTrades: 'Trades',
    colWin: 'Win%',
    statTrades: '{n} trades',
    statWins: '{n} wins',
    statAvg: 'avg',
  },
  whatif: {
    blurb:
      'Deterministic backtest grid (paths/PnL computed in Rust) over the audit corpus. On-demand. ⚠️ The backtest does NOT model live trailing (advice/min_hold) — trust the entry axes; validate trailing on the Live tab.',
    kpiCorpus: 'Corpus ticks',
    kpiBaseline: 'Baseline net PnL',
    kpiWin: 'Win rate',
    kpiClosed: 'Closed',
    recTitle: 'Recommendation · apply manually (no auto-apply)',
    recBest:
      'Best alpha variant with a positive delta on the corpus. Exposure variants (size-only) are never recommended as tuning.',
    recNone:
      'No alpha improvement on the current corpus — keep the config. (Exposure variants do not count as a strategy improvement.)',
    gridTitle: 'Deterministic grid · sorted by Δ net PnL (explicit sign)',
    colAxis: 'Axis',
    colValue: 'Value',
    colClass: 'Class',
    colDelta: 'Δ net PnL',
    colNet: 'net PnL',
    colWL: 'W/L',
    subTitle:
      'Token substitution · hypothesis (candidates have no corpus, not backtested)',
    subWorst: 'Worst symbol: {symbol}',
    subPool: 'Substitution pool (disabled, validate in paper first):',
    subNone: 'No drop candidate with enough trades.',
    empty: 'Click “Generate” to run the tuning grid.',
  },
  positions: {
    title: 'Open Positions',
    titleCount: 'Open Positions ({n})',
    flatNoPos: 'Flat — no open positions',
    flatGuards: 'Flat — guards holding {n} setups',
    flatNote:
      'The strategy is monitoring the market; entries open when exchange consensus and the entry guards align.',
    stop: 'stop',
    peak: 'peak',
    target: 'target',
    trail: 'trail',
    trailArmed: '🔒 trail armed',
    trailLocked: '🔒 locks {pct}',
    tpAt: 'TP at {pct}',
    tpBeyond: 'TP ✓ {pct} beyond',
    tpMax: ' · max {pct}',
    cushion: 'cushion to stop',
    entry: 'entry',
    mark: 'mark',
    tpArm: 'TP-arm',
  },
  console: {
    connecting: 'Connecting to ViperTrade',
    // KPI strip
    equity: 'Equity',
    delta24h: '24h',
    winRate: 'Win rate',
    open: 'Open',
    today: 'Today',
    trades: 'trades',
    // Market sentiment
    sentimentTitle: 'Market Sentiment',
    fngTitle: 'Fear & Greed Index',
    indexUnavailable: 'Index unavailable',
    fngBlurb:
      'The Crypto Fear & Greed Index distills volatility, momentum, volume, social media, and surveys into a single 0–100 reading of market emotion.',
    fngExtremeFear: 'Extreme Fear',
    fngExtremeFearNote:
      'Investors are highly fearful — often an oversold zone with potential buying opportunities.',
    fngFear: 'Fear',
    fngFearNote: 'Caution dominates the market — sentiment leans bearish.',
    fngNeutral: 'Neutral',
    fngNeutralNote: 'Sentiment is balanced — no strong directional bias.',
    fngGreed: 'Greed',
    fngGreedNote:
      'Optimism is rising — momentum is bullish, but watch for froth.',
    fngExtremeGreed: 'Extreme Greed',
    fngExtremeGreedNote:
      'Markets are euphoric — historically a zone of elevated pullback risk.',
    longShortRatio: 'Long/Short ratio',
    bybitPerps: 'Bybit perpetuals',
    long: 'Long',
    short: 'Short',
    longShortUnavailable: 'Long/Short ratio unavailable',
    // Equity curve
    equityTitle: 'Equity curve',
    equitySubtitle:
      'Cumulative realized PnL across {n} closed trades · marker per fill',
    equityNet: 'net',
    equityPeak: 'peak',
    equityEmpty: 'No closed trades yet — the curve plots as fills land.',
    fill: 'fill',
    equityLabel: 'equity',
  },
  trades: {
    title: 'Trades',
    subtitle:
      'The ledger with PnL attribution by close-reason — where the edge and the bleed come from.',
    net: 'net',
    closed: 'closed',
    win: 'win',
    // filters
    fSymbol: 'Symbol',
    fSide: 'Side',
    fStatus: 'Status',
    fFrom: 'From',
    fTo: 'To',
    all: 'All',
    long: 'Long',
    short: 'Short',
    statusClosed: 'Closed',
    statusOpen: 'Open',
    rangeToday: 'Today',
    range7d: '7d',
    range30d: '30d',
    clearDates: 'clear dates ✕',
    reasonChip: 'reason: {reason} ✕',
    // table
    colAsset: 'Asset',
    colSide: 'Side',
    colPnl: 'PnL',
    colEntry: 'Entry',
    colExit: 'Exit',
    colReason: 'Reason',
    colClosed: 'Closed',
    colHeld: 'Held',
    rowOpen: 'open',
    emptyFiltered: 'No trades match the current filters.',
    pageInfo: '{n} trades · page {page}/{total}',
    prev: 'Prev',
    next: 'Next',
    // attribution cards
    attributionTitle: 'PnL by close reason',
    cardTrades: '{n} trades · {win}% win',
    cardTrade: '{n} trade · {win}% win',
    // close reasons (fallback titleCases unknown values)
    reasonTrailingStop: 'Trailing Stop',
    reasonStopLoss: 'Stop Loss',
    reasonThesisInvalidated: 'Thesis Invalidated',
    reasonTakeProfit: 'Take Profit',
    reasonManual: 'Manual',
    reasonLiquidation: 'Liquidation',
    reasonBreakEven: 'Break Even',
    reasonUnknown: 'Unknown',
  },
  system: {
    title: 'System',
    subtitlePre:
      'Runtime controls and service health — operate the strategy without reaching for ',
    subtitlePost: '.',
    opDisabled:
      'Operator controls disabled — set OPERATOR_API_TOKEN (auth mode "token") on the api.',
    opReadonly: 'Controls below are read-only until then.',
    runtime: 'Runtime',
    tradingMode: 'Trading mode',
    tradingModeHint: 'Set at deploy (env) — read-only here',
    killSwitch: 'Kill-switch',
    killTrippedBy: 'Tripped by {actor} — trading blocked',
    killTripped: 'Tripped — trading blocked',
    killArmed: 'Armed — trading allowed',
    stTripped: 'TRIPPED',
    stArmed: 'ARMED',
    restore: 'Restore',
    restoreTrading: 'Restore trading',
    tripKill: 'Trip kill-switch',
    tripNow: 'Trip now',
    executor: 'Executor',
    executorProcessing: 'Processing decisions',
    executorPaused: 'Paused',
    stOn: 'ON',
    stOff: 'OFF',
    disable: 'Disable',
    enable: 'Enable',
    disableExecutor: 'Disable executor',
    enableExecutor: 'Enable executor',
    realDecisionsHint:
      'Runtime configmap patch — not exposed by the API; apply via kubectl',
    copyCommand: 'Copy command',
    copiedCmd: 'Copied ✓',
    serviceHealth: 'Service health',
    noServiceHealth: 'No service health reported.',
    // service-flow diagram
    flowStage: 'Stage',
    flowLatency: 'Latency',
    flowRole: 'Role',
    flowDown: 'down',
    flowSlow: 'slow',
    flowLive: 'live',
    flowSources: 'Sources',
    flowExchangeFeeds: 'Exchange Feeds',
    flowExecutionVenue: 'Execution Venue',
    flowVenues3: '3 venues',
    flowSingleVenue: 'single venue',
    flowMarketFlow: 'market flow',
    flowFanOut: 'execution fan-out',
    flowCorePipeline: 'Core pipeline',
    flowMarketToExec: 'Market Data to Execution',
    flowRuntimePath: 'runtime path',
    flowStageMarket: 'Market',
    flowStageStrategy: 'Strategy',
    flowStageExecutor: 'Executor',
    flowRoleNormalize: 'normalize + publish',
    flowRoleDecide: 'score + decide',
    flowRoleOrders: 'orders + reconcile',
    flowRoleOrdersPaused: 'orders paused',
    flowVenueMulti: 'multi venue',
    flowVenueSingle: 'single venue',
    flowSymbolsLine: '{n} active symbols · {venue}',
    flowSymbolLine: '{n} active symbol · {venue}',
    flowPositionsLine: '{n} open positions',
    flowPositionLine: '{n} open position',
    flowScanning: 'Scanning market conditions',
    flowScanningState: 'Scanning',
    flowClosedObserved: '{n} closed trades observed',
    flowAwaiting: 'Awaiting valid execution pressure',
    flowFeedDegraded: 'Feed normalization degraded',
    flowVenueTelemetry: 'Venue telemetry is flowing into the runtime path',
    flowSidecars: 'Sidecars',
    flowStateAnalysis: 'State and Analysis',
    flowObservers: 'observers',
    flowIdle: 'idle',
  },
  strategy: {
    title: 'Strategy',
    live: 'live',
    polling: 'polling',
    subtitle:
      'Per-symbol multi-exchange consensus, entry guards, and the reason each symbol is entering or being held.',
    symbols: 'symbols',
    entering: 'entering',
    guarded: 'guarded',
    legendGuard: '%B guard zone (≥{ceil} / ≤{floor})',
    legendWeak: 'ADX < 20 weak trend',
    legendStrong: 'ADX ≥ 25 strong trend',
    empty:
      'No decisions recorded yet — the strategy publishes one per symbol as market data flows in.',
    colSymbol: 'Symbol',
    colState: 'State',
    colConsensus: 'Consensus',
    colRsi: 'RSI',
    colPb: '%B',
    colAdx: 'ADX',
    colWhy: 'Why',
    na: 'n/a',
    whyEnter: 'Entering {dir} — consensus {side}, entry guards clear.',
    whyHolding: 'Holding — {reasons}.',
    whyHoldingDefault:
      'Holding — consensus {side}, conditions not yet aligned.',
    rNoConsensus: 'no directional consensus',
    rPbLong: '%B {pb} > {ceil} guards longs',
    rPbShort: '%B {pb} < {floor} guards shorts',
    rWeakTrend: 'weak trend (ADX {adx})',
    rMacd: 'MACD has not crossed in favour',
    rVolume: '24h volume below minimum',
    rRegime: 'neutral consensus regime',
    rDirectional: 'directional checks did not pass',
    rRsi: 'RSI outside the entry band',
    rRiskGate: 'account risk limit reached',
    rMaxOpen: 'max concurrent positions reached',
    rRuntimeOff: 'scalp off — no entry evaluation',
    sideBullish: 'bullish',
    sideBearish: 'bearish',
    sideNeutral: 'neutral',
  },
  auth: {
    loginTitle: 'ViperTrade — Local Access',
    username: 'Username',
    password: 'Password',
    signIn: 'Sign In',
    signingIn: 'Signing in...',
    invalidCredentials: 'Invalid credentials',
    authFailed: 'Authentication failed',
    localDevPre: 'Local development — use ',
    logout: 'Logout',
    loggingOut: 'Logging out...',
  },
  cmd: {
    goConsole: 'Go to Console',
    goStrategy: 'Go to Strategy',
    goTrades: 'Go to Trades',
    goAnalysis: 'Go to Analysis',
    goSystem: 'Go to System · controls',
    placeholder: 'Jump to…',
    noMatches: 'No matches',
  },
  app: {
    dashboardError: 'Dashboard Error',
    retry: 'Retry',
    checkApi: 'Check API',
    somethingWrong: 'Something went wrong!',
    tryAgain: 'Try again',
    loadingApp: 'Loading ViperTrade...',
    notFoundTitle: 'Page Not Found',
    notFoundBody: "The page you're looking for doesn't exist.",
    goDashboard: 'Go to Dashboard',
    healthLive: 'LIVE',
    healthConnecting: 'CONNECTING',
    healthStale: 'STALE',
    healthOffline: 'OFFLINE',
    healthFeed: 'Realtime feed: {status}',
  },
};

// Value types widen to `string` (no `as const`) so pt-BR.ts can hold translated
// strings while still being checked against the same key shape.
export type Messages = typeof en;
export default en;
