...
use state::{
    ...
    ConfigPromoteRequest, ConfigSaveRequest, ConfigSuggestRequest, ConfigValidateSymbolRequest,
...
// ───────────────────────── trading config (Phase 3) ─────────────────────────

/// PUBLISH viper:config_changed so the running strategy hot-reloads. Non-fatal.
async fn publish_config_changed(redis_url: &str, version_id: i64) {
    let res: redis::RedisResult<()> = async {
        let client = redis::Client::open(redis_url)?;
        let mut conn = client.get_multiplexed_async_connection().await?;
        let _: () = redis::cmd("PUBLISH")
            .arg("viper:config_changed")
            .arg(version_id)
    ...
    .await;
    if let Err(err) = res {
        tracing::warn!(error = %err, "Failed to publish viper:config_changed");
    }
}

/// Insert `config` as a NEW active version in one txn: link parent = the current
/// active id, deactivate it, insert the new row active. Returns the new id.
/// Shared by save + promote (and the Phase 4 "Aplicar" path).
async fn insert_active_version(
    pool: &PgPool,
    created_by: &str,
    note: &str,
    source_review_id: Option<i64>,
    config: &Value,
) -> Result<i64, sqlx::Error> {
    let mut tx = pool.begin().await?;
    let parent_id: Option<i64> =
        sqlx::query_as::<_, (i64,)>("SELECT id FROM trading_config_versions WHERE active LIMIT 1")
            .fetch_optional(&mut *tx)
            .await?
            .map(|(id,)| id);
    sqlx::query("UPDATE trading_config_versions SET active=false WHERE active")
        .execute(&mut *tx)
        .await?;
    let (id,): (i64,) = sqlx::query_as(
        "INSERT INTO trading_config_versions \
         (created_by, parent_id, source_review_id, note, config, active) \
...
/// Activate (or revert to) an existing version by id. Returns false if not found.
async fn activate_version(pool: &PgPool, version_id: i64) -> Result<bool, sqlx::Error> {
    let mut tx = pool.begin().await?;
    let exists = sqlx::query_as::<_, (i64,)>("SELECT id FROM trading_config_versions WHERE id=$1")
        .bind(version_id)
        .fetch_optional(&mut *tx)
        .await?;
    if exists.is_none() {
    ...
    sqlx::query("UPDATE trading_config_versions SET active=false WHERE active")
        .execute(&mut *tx)
        .await?;
    sqlx::query("UPDATE trading_config_versions SET active=true WHERE id=$1")
        .bind(version_id)
        .execute(&mut *tx)
        .await?;
    tx.commit().await?;
...
/// GET /api/v1/config — the active version (full config) + the version list
/// (metadata only), newest first.
async fn config_handler(state: Arc<AppState>) -> impl Reply {
    let Some(pool) = &state.db_pool else {
        return json_err(
            StatusCode::SERVICE_UNAVAILABLE,
            "db_unavailable",
    ...
    let active = sqlx::query_as::<_, (i64, DateTime<Utc>, String, Option<String>, String)>(
        "SELECT id, created_at, created_by, note, config::text \
         FROM trading_config_versions WHERE active LIMIT 1",
    )
    .fetch_optional(pool)
    .await;
    let versions = sqlx::query_as::<_, (i64, DateTime<Utc>, String, Option<String>, bool)>(
        "SELECT id, created_at, created_by, note, active \
         FROM trading_config_versions ORDER BY id DESC LIMIT 100",
    )
    .fetch_all(pool)
    .await;

    match (active, versions) {
        (Ok(active), Ok(versions)) => {
            let active_json = active.map(|(id, created_at, created_by, note, cfg)| {
                json!({
                    "id": id, "created_at": created_at, "created_by": created_by,
                    "note": note,
                    "config": serde_json::from_str::<Value>(&cfg).unwrap_or(Value::Null),
                })
            });
            let items: Vec<Value> = versions
                .into_iter()
                .map(|(id, created_at, created_by, note, active)| {
                    json!({
                        "id": id, "created_at": created_at, "created_by": created_by,
                ...
                .collect();
            json_ok(&json!({ "active": active_json, "versions": items }))
        }
        (Err(e), _) | (_, Err(e)) => json_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "query_failed",
            e.to_string(),
        ),
...
/// GET /api/v1/config/versions/:id — one version's full config (drives the diff).
async fn config_version_handler(id: i64, state: Arc<AppState>) -> impl Reply {
    let Some(pool) = &state.db_pool else {
        return json_err(
            StatusCode::SERVICE_UNAVAILABLE,
            "db_unavailable",
    ...
    let row = sqlx::query_as::<_, (i64, DateTime<Utc>, String, Option<String>, bool, String)>(
        "SELECT id, created_at, created_by, note, active, config::text \
             FROM trading_config_versions WHERE id=$1",
    )
    .bind(id)
    .fetch_optional(pool)
    .await;
    match row {
        Ok(Some((id, created_at, created_by, note, active, cfg))) => json_ok(&json!({
            "id": id, "created_at": created_at, "created_by": created_by,
            "note": note, "active": active,
            "config": serde_json::from_str::<Value>(&cfg).unwrap_or(Value::Null),
        })),
...
/// POST /api/v1/config — save a full edited config as a new active version.
async fn config_save_handler(
        ...
        .unwrap_or(false)
    {
        return json_err(
            StatusCode::BAD_REQUEST,
            "invalid_config",
            "config must be an object with a 'global' key",
        );
    }
    let note = req
        .note
        .clone()
        .unwrap_or_else(|| "operator edit".to_string());
    match insert_active_version(pool, "operator", &note, None, &req.config).await {
        Ok(id) => {
            publish_config_changed(&state.redis_url, id).await;
            json_ok(&json!({ "id": id, "active": true }))
        }
        Err(e) => json_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "persist_failed",
...
/// POST /api/v1/config/activate — activate (or revert to) a version by id.
async fn config_activate_handler(
    req: ConfigActivateRequest,
    token_header: Option<String>,
    _operator_id_header: Option<String>,
    state: Arc<AppState>,
) -> impl Reply {
    ...
    match activate_version(pool, req.version_id).await {
        Ok(true) => {
            publish_config_changed(&state.redis_url, req.version_id).await;
            json_ok(&json!({ "id": req.version_id, "active": true }))
        }
        Ok(false) => json_err(
            StatusCode::NOT_FOUND,
            "not_found",
...
/// POST /api/v1/config/promote — copy the active PAPER profile into MAINNET as a
/// new active version (explicit, audited; no silent anchor inheritance).
async fn config_promote_handler(
    req: ConfigPromoteRequest,
    token_header: Option<String>,
    _operator_id_header: Option<String>,
    state: Arc<AppState>,
) -> impl Reply {
    ...
    let mut config: Value = match active {
        Ok(Some((cfg,))) => match serde_json::from_str(&cfg) {
            Ok(v) => v,
            Err(e) => {
                return json_err(
                    StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let Some(paper) = config.pointer("/global/mode_profiles/PAPER").cloned() else {
        return json_err(
            StatusCode::BAD_REQUEST,
            "no_paper",
            "active config has no global.mode_profiles.PAPER",
        );
    };
    if let Some(mp) = config
        .pointer_mut("/global/mode_profiles")
        .and_then(Value::as_object_mut)
    {
        mp.insert("MAINNET".to_string(), paper);
    }
    let note = req
        .note
        .clone()
        .unwrap_or_else(|| "promote PAPER->MAINNET".to_string());
...
pub async fn run() {
    ...
    // Trading config (Phase 3): read is open; writes are operator-gated.
    ...
    let config_version = api_v1
        ...
        .and(warp::get())
        .and(with_state(state.clone()))
        .then(config_version_handler);

    let config_save = api_v1
        .and(warp::path("config"))
        .and(warp::path::end())
        .and(warp::post())
        .and(warp::body::json::<ConfigSaveRequest>())
        .and(warp::header::optional::<String>("x-operator-token"))
        .and(warp::header::optional::<String>("x-operator-id"))
        .and(with_state(state.clone()))
        .then(config_save_handler);

    let config_activate = api_v1
        .and(warp::path("config"))
        .and(warp::path("activate"))
        .and(warp::path::end())
        .and(warp::post())
        .and(warp::body::json::<ConfigActivateRequest>())
        .and(warp::header::optional::<String>("x-operator-token"))
        .and(warp::header::optional::<String>("x-operator-id"))
        .and(with_state(state.clone()))
        .then(config_activate_handler);

    let config_promote = api_v1
        .and(warp::path("config"))
        .and(warp::path("promote"))
        .and(warp::path::end())
        .and(warp::post())
        .and(warp::body::json::<ConfigPromoteRequest>())
        .and(warp::header::optional::<String>("x-operator-token"))
        .and(warp::header::optional::<String>("x-operator-id"))
        .and(with_state(state.clone()))
