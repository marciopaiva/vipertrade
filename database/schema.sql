...
CREATE TABLE trades (
    ...
    close_reason TEXT CHECK (close_reason IN ('take_profit', 'stop_loss', 'trailing_stop', 'time_exit', 'manual', 'liquidation', 'circuit_breaker', 'thesis_invalidated', 'error')),
...
-- ==========================================================
-- TABLE 3.2: strategy_decision_audit (rich decision diagnostics)
-- ==========================================================

CREATE TABLE strategy_decision_audit (
...
CREATE INDEX idx_strategy_decision_audit_created_at
    ON strategy_decision_audit(created_at);
CREATE INDEX idx_strategy_decision_audit_symbol_created_at
    ON strategy_decision_audit(symbol, created_at DESC);
CREATE INDEX idx_strategy_decision_audit_action_created_at
    ON strategy_decision_audit(action, created_at DESC);
CREATE INDEX idx_strategy_decision_audit_status_created_at
    ON strategy_decision_audit(executor_status, created_at DESC);
CREATE INDEX idx_strategy_decision_audit_reason
    ON strategy_decision_audit(reason);
CREATE INDEX idx_strategy_decision_audit_payload_gin
...
COMMENT ON VIEW trade_net IS 'Resultado líquido (pnl - fees - funding) por trade fechado em paper';
