...
```bash
${CONTAINER_ENGINE:-podman} exec -i vipertrade-postgres psql -U viper -d vipertrade <<'SQL'
...
```bash
${CONTAINER_ENGINE:-podman} exec -i vipertrade-postgres psql -U viper -d vipertrade -At -F '|' -c \
